# Migrating Expo UI to SSPL Design System

Read `artifacts/sspl-design-system/docs/AGENTS.md` and
`artifacts/sspl-design-system/docs/consuming-expo.md` first. Use this guide
...
- Rewrite color/theme imports to
  `@workspace/sspl-design-system/lib/native-theme` and
  `@workspace/sspl-design-system/hooks/use-colors`.
- Rewrite the root layout to use
  `@workspace/sspl-design-system/hooks/use-fonts` while preserving its
  SplashScreen gating.
...
and hook directly from `@workspace/sspl-design-system` and retains only
